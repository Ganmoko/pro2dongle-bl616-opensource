...
> 简单来说：Pro2 不能像普通蓝牙手柄一样，直接在 Windows 的蓝牙设置中配对
> 使用。本固件让 LCTech BL616 通过 BLE 连接 Pro2，再通过 USB 将手柄输入、
> 体感和 HD 振动桥接给电脑或其他主机。
...
这里的 Pro2 指 Nintendo Switch 2 Pro Controller（USB `VID:PID 057e:2069`）。

## 为什么需要 BL616

Pro2 的无线通信虽然使用 BLE，但它不是 Windows 能按普通蓝牙手柄原生配对和
使用的标准 Bluetooth HID 设备。Windows“添加设备”无法直接完成所需的专有
配对和初始化；手柄输入也通过 Nintendo 的自定义 GATT 通知传输，而不是由
Windows 作为普通蓝牙 HID 输入设备提供。

本固件让 BL616 充当 Pro2 的专用无线接收器：BL616 负责 BLE 扫描、专有配对、
GATT 初始化、输入通知和振动通信，再向电脑或主机提供 Nintendo 风格的
`057e:2069` USB 设备。也就是说，它解决的是“Pro2 无法作为普通蓝牙手柄直接
连接电脑使用”的问题。

专门实现 Pro2 私有协议的电脑软件也可以直接访问其 BLE 服务，但这与 Windows
原生蓝牙配对不同；本项目选择把整套无线协议放在 BL616 上完成。
